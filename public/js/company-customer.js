class CompanyCustomerManager {
    ...
    removeDevice(deviceId) {
        if (this.devices.length > 1) {
            ...
            alert('يجب أن يكون هناك على الأقل لابتوب واحد في طلب الشركة');
        }
    }
    ...
    renderDevices() {
        ...
        container.innerHTML = this.devices.map((device, index) => `
            <div class="device-card" style="background: rgba(16, 185, 129, 0.1); padding: 1.5rem; border-radius: 12px; margin-bottom: 1rem; border: 1px solid rgba(16, 185, 129, 0.2);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    <h4 style="color: #10b981; margin: 0;"><i class="fas fa-laptop"></i> اللابتوب #${index + 1}</h4>
                    ${this.devices.length > 1 ? `
                        <button type="button" onclick="companyCustomerManager.removeDevice(${device.id})" class="btn btn-danger" style="padding: 0.25rem 0.5rem; font-size: 0.8rem;">
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            // Create company request with all devices
            const requestData = {
                ...
                deviceCount: companyData.deviceCount,
                devices: devicesData,
                status: 'Received',
                priority: 'Medium'
            };
